...
# entry's exact pins (valiss from the frozen v0.8.0 tag, httpx and
# cryptography per pyproject.toml) and run smoke_test.py, which builds the
# go-0.12 reference server from the sibling entry, starts it against the
# committed fixture, and drives every applicable scenario from scenarios.yaml
# through this entry's client over HTTP. Requires a Go toolchain for the
# reference server.
#
...
if ! command -v go >/dev/null 2>&1; then
    echo "smoke: go not found (needed to build the go-0.12 reference server)" >&2
    exit 1
fi
