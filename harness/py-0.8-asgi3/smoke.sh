...
# entry's exact pins (valiss from the frozen v0.8.0 tag, starlette, uvicorn
# and cryptography per pyproject.toml) and run smoke_test.py, which starts
# the entry's server against the committed fixture and drives every
# applicable scenario from scenarios.yaml through the go-0.12 reference
# client over HTTP. Requires a Go toolchain for the reference client.
#
...
if ! command -v go >/dev/null 2>&1; then
    echo "smoke: go not found (needed to build the go-0.12 reference client)" >&2
    exit 1
fi
